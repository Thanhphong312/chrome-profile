...
export default function ProfileCard({ profile, onEdit, onDelete }) {
  ...
  // Poll running status every 2 seconds
  useEffect(() => {
    let cancelled = false
  ...
  async function handleDelete() {
    if (!window.confirm(`Delete profile "${profile.name}"? This will remove its Chrome data folder.`)) return
  ...
  return (
    <div className={`bg-gray-800 border rounded-xl p-5 flex flex-col gap-3 transition-colors ${
      ...
      {error && (
        <p className="text-xs text-red-400 bg-red-900/30 px-2 py-1 rounded">{error}</p>
      )}
